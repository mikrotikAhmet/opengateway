        ...
        <div class="panel-body">
            <div class="well">
                <div class="row">
                    ...
                    <div class="col-sm-3">
                        <div class="form-group">
                            <label class="control-label" for="input-account-group"><?php echo $entry_account_group; ?></label>
                            <select name="filter_account_group_id" id="input-account-group" class="form-control">
                                <option value="*"></option>
                                <?php foreach ($account_groups as $account_group) { ?>
                                <?php if ($account_group['account_group_id'] == $filter_account_group_id) { ?>
                    ...
                    <table class="table table-bordered table-hover">
                        ...
                        <tr>
                            ...
                            <td class="text-left"><?php if ($sort == 'c.email') { ?>
                                ...
                                <?php } ?></td>
                            <td class="text-left"><?php if ($sort == 'account_group') { ?>
                                <a href="<?php echo $sort_account_group; ?>" class="<?php echo strtolower($order); ?>"><?php echo $column_account_group; ?></a>
                                <?php } else { ?>
                                <a href="<?php echo $sort_account_group; ?>"><?php echo $column_account_group; ?></a>
                                <?php } ?></td>
                        ...
                        <tr>
                            ...
                            <td class="text-left"><?php echo $account['account_group']; ?></td>
                            <td class="text-left"><?php echo $account['status']; ?></td>
                            <td class="text-left"><?php echo $account['ip']; ?></td>
                            <td class="text-left"><?php echo $account['date_added']; ?></td>
                            <td class="text-right"><?php if (!$account['approved']) { ; ?>
                                <a href="<?php echo $account['approve']; ?>" data-toggle="tooltip" title="<?php echo $button_approve; ?>" class="btn btn-xs btn-success"><i class="fa fa-thumbs-o-up"></i></a>
                                <?php } else { ?>
                                <button type="button" class="btn btn-xs btn-success" disabled><i class="fa fa-thumbs-o-up"></i></button>
                                <?php } ?>
                                <div class="btn-group" data-toggle="tooltip" title="<?php echo $text_login; ?>">
                                    <button type="button"class="btn btn-xs btn-info"><i class="fa fa-lock"></i></button>
                                </div>
                                <a href="<?php echo $account['edit']; ?>" data-toggle="tooltip" title="<?php echo $button_edit; ?>" class="btn btn-xs btn-primary"><i class="fa fa-pencil"></i></a></td>
                        </tr>
                        <?php } ?>
                        <?php } else { ?>
                        <tr>
...
<script type="text/javascript"><!--
    $('#button-filter').on('click', function() {
        url = 'index.php?route=account/account&token=<?php echo $token; ?>';
        ...
        var filter_account_group_id = $('select[name=\'filter_account_group_id\']').val();

        if (filter_account_group_id != '*') {
            url += '&filter_account_group_id=' + encodeURIComponent(filter_account_group_id);
        }
...
<script type="text/javascript"><!--
    $('input[name=\'filter_name\']').autocomplete({
        'source': function(request, response) {
            $.ajax({
                url: 'index.php?route=account/account/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
                dataType: 'json',
    ...
    $('input[name=\'filter_email\']').autocomplete({
        'source': function(request, response) {
            $.ajax({
                url: 'index.php?route=account/account/autocomplete&token=<?php echo $token; ?>&filter_email=' +  encodeURIComponent(request),
                dataType: 'json',
                success: function(json) {
                    response($.map(json, function(item) {
                        return {
