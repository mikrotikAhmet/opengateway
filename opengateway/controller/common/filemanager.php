...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		if (isset($this->request->get['directory'])) {
			$directory = rtrim(DIR_IMAGE . 'crm/' . str_replace(array('../', '..\\', '..'), '', $this->request->get['directory']), '/');
		} else {
			$directory = DIR_IMAGE . 'crm';
		}
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'href'  => $this->url->link('common/filemanager', 'token=' . $this->session->data['token'] . '&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'crm/'))) . $url, 'SSL')
				);
